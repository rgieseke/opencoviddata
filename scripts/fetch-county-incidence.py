...
def fetch_county_incidence(county):
...
# API-Download für Covid19-Inzidenzen in {county}, letzte KW ggf. noch nicht vollständig
"""
    with open(
        root
        / f"data/counties/survstat-covid19-incidence-{ county.lower().replace(' ', '-') }.csv",
        "w",
    ) as f:
        f.write(header)
        f.write(df.to_csv())
